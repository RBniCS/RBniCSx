...
def pytest_ignore_collect(path, config):
    """Ignore .py files generated by previous runs."""
...
def pytest_collect_file(path, parent):
    ...
    if path.ext == ".ipynb":
        ...
        if config.getoption("--flake8"):
            ...
            exporter = nbconvert.exporters.PythonExporter(filters=filters)
            exporter.exclude_input_prompt = True
            code, _ = exporter.from_filename(path)
            code = code.replace("# coding: utf-8", f'# coding: utf-8\n"""Automatically generated from {path}."""')
            code = code.rstrip("\n") + "\n"
...
def pytest_pycollect_makemodule(path, parent):
    ...
    This is necessary because they may get out of sync with the corresponding .ipynb file.
    """
    if path.ext == ".py":
        assert not path.new(ext=".ipynb").exists(), "Please run pytest on jupyter notebooks, not plain python files."
        return DoNothingFile.from_parent(parent=parent, fspath=path)
