...
def test_cpp_backends_frobenius_inner_product() -> None:
    ...
    A = petsc4py.PETSc.Mat().createDense((2, 3), comm=mpi4py.MPI.COMM_WORLD)  # type: ignore[arg-type]
    A.setUp()
    B = petsc4py.PETSc.Mat().createDense((2, 3), comm=mpi4py.MPI.COMM_WORLD)  # type: ignore[arg-type]
    B.setUp()
    for i in range(2):
        for j in range(3):
